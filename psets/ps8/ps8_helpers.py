...
def generate_random_graph_gamma(Graph, n, gamma, seed=10):
    random.seed(seed)
    p = gamma / n
    g = Graph(n)
    for u in range(n):
...
def generate_hard_instance_from_coloring(Graph, n, eta, seed=10):
    random.seed(seed)
    g = Graph(n)
    for n in range(g.N): 
        g.colors[n] = random.randint(0, 2) # mchoose random col between 0, 1, 2
    
    for u in range(n):
        for v in range(u + 1, n):
            if g.colors[v]!= g.colors[u]:
                if random.random() < (eta / n):
                    g.add_edge(u, v)
    return g


def load_dimacs_graph(Graph, path):
    n = 0
    edges = []
    with open(path) as f:
        for line in f:
